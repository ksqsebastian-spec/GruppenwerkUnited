...
import type { Stelle, EmpfehlungWithStelle, EmpfehlungStatus, AppSettings } from "@/types";
...
export default function StellenPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [stellenRes, empRes] = await Promise.all([
        fetch("/api/admin/stellen"),
        fetch("/api/admin/stellen?view=empfehlungen&pageSize=100"),
      ]);
      if (stellenRes.ok) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/admin/stellen", {
        method: "POST",
  ...
  async function handleDelete(stelle: Stelle) {
    ...
    try {
      const res = await fetch(`/api/admin/stellen?id=${stelle.id}`, { method: "DELETE" });
      if (!res.ok) {
        const data = await res.json();
