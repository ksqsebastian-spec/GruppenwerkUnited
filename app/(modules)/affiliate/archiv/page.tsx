...
export default function ArchivPage(): React.JSX.Element {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setEmpfehlungen([]);
      setError("Das Archiv konnte nicht geladen werden. Bitte versuche es erneut.");
    } finally {
      setLoading(false);
    }
  }, [page, search]);
  ...
  return (
    <div className="animate-fadeIn flex flex-col gap-8">
      ...
      <div className="grid grid-cols-2 gap-px bg-border rounded-xl overflow-hidden border border-border">
        ...
        <StatCard label="Ausgezahlt gesamt" value={formatCurrency(totalProvision)} />
      </div>
              ...
              empfehlungen.map((emp) => (
                <tr key={emp.id} className="border-t border-border hover:bg-muted/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <div className="font-semibold text-foreground">{emp.handwerker?.name ?? emp.kunde_name}</div>
                    <div className="text-xs text-muted-foreground">{emp.handwerker?.email ?? ""}</div>
