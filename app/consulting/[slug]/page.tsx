...
export default function ConsultingCompanyPage(): React.JSX.Element {
  ...
  const totals = (categories ?? []).reduce(
    (acc: { green: number; orange: number; red: number }, cat: ConsultingCategoryWithCheckpoints) => ({
    ...
    return (
      <div className="max-w-3xl mx-auto flex flex-col gap-3">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="h-14 rounded-xl border border-[#e5e5e5] bg-[#f5f5f5] animate-pulse" />
        ))}
      </div>
    );
  }
