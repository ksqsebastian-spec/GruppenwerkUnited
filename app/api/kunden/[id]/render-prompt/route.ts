...
export async function POST(
  ...
  let body: { prompt_id?: unknown; encode?: unknown };
  try {
    body = (await req.json()) as { prompt_id?: unknown; encode?: unknown };
  } catch {
  ...
  const encode = body.encode === true;

  try {
    const result = await renderPrompt(body.prompt_id, customerId, session.companyId, { encode });
