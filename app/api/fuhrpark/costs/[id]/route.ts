...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    const row = await fetchCost(id);
    return NextResponse.json(row);
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Kosteneintrag nicht gefunden' }, { status: 404 });
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    const row = await updateCost(id, body as Parameters<typeof updateCost>[1]);
    return NextResponse.json(row);
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Kosteneintrag konnte nicht aktualisiert werden' }, { status: 500 });
  }
}
