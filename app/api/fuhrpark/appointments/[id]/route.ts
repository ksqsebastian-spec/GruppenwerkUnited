...
export async function GET(_: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Termin nicht gefunden' }, { status: 404 });
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  try {
    const row = await updateAppointment(id, body as Parameters<typeof updateAppointment>[1]);
    return NextResponse.json(row);
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : 'Termin konnte nicht aktualisiert werden' }, { status: 500 });
  }
}
