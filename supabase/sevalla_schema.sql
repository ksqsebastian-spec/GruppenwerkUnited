-- ============================================================================
-- GruppenwerkUnited — Werkbank Schema für Sevalla PostgreSQL 17
-- Angepasst aus complete_schema.sql (Supabase-spezifische Teile entfernt)
-- ============================================================================
...
-- Affiliate: Handwerker/Partner (auth_user_id ohne FK — kein Supabase Auth)
...
SELECT
  t.id AS tender_id, t.title, t.authority, t.deadline, t.deadline_date,
  t.category, t.url, t.status, t.requested, t.created_at,
  s.calendar_week, s.year, s.scan_date, s.report_url,
  c.slug AS company_slug, c.name AS company_name, c.color AS company_color,
  m.relevance, m.reason,
  CASE
...
SELECT
  c.name AS company_name, c.slug AS company_slug, c.color,
  s.calendar_week, s.year, s.scan_date,
...
SELECT
  ws.company_name, ws.company_slug, ws.color,
  ws.calendar_week, ws.year, ws.scan_date, ws.tender_count,
  LAG(ws.tender_count) OVER (PARTITION BY ws.company_slug ORDER BY ws.year, ws.calendar_week) AS prev_week_count,
  ws.tender_count - COALESCE(LAG(ws.tender_count) OVER (PARTITION BY ws.company_slug ORDER BY ws.year, ws.calendar_week), ws.tender_count) AS week_change
